...
let net = new Network(inputs, [256,512,256, inputs.length])
...
function fmain(){
    ...
    if (keysPressed[' '] ) {
        ...
        for (let t = 0; t < (9100); t++) {
            ...
            let x = (Math.floor(t/91)%10)-5
            let y = (Math.floor(t/910)%10)-5
            canvas_context.drawImage(fruiw, (t % (91)) * 270, 0, 270, 270, x, y, 32, 32)
            ...
            canvas_context.drawImage(frui, (t % (91)) * 270, 0, 270, 270, x,y, 32, 32)
            pixboxes[t] = canvas_context.getImageData(0, 0, 32, 32)
    ...
    for (let t = 0; t < statpixboxes2[count].data.length; t += 3) {
        ...
        statpixboxes2[count].data[k] = net.outputs[t] * 255
        statpixboxes2[count].data[k + 1] = net.outputs[t + 1] * 255
        statpixboxes2[count].data[k + 2] = net.outputs[t + 2] * 255
        statpixboxes2[count].data[k + 3] = 255
    }

        // canvas_context.putImageData(pix3,64+( (count % 13) * 32),64+( Math.floor(count / 13) * 32))
        // statpixboxes[count] = canvas_context.getImageData(0, 0, 32, 32)

        // canvas_context.fillStyle = "#888888"
        // canvas_context.fillRect(32*(count%13), 0, 32, 32)
        canvas_context.putImageData(statpixboxes2[count], 32*(count%91), 64+(32*(Math.floor(count/91))))
        // canvas_context.putImageData(statpixboxes[count], 32*(count%91), 32+(96*(Math.floor(count/91))))
        canvas_context.putImageData(pixboxes[count], 32*(count%91), 0)
        // canvas_context.drawImage(guys[count], 0, 0, 64, 64, 32, 0, 32, 32)

        // canvas_context.drawImage(fruiw, (count%(91))*270, 0, 270,270, 32*(count%91), 32+(96*(Math.floor(count/91))), 32, 32) 
        // canvas_context.drawImage(frui, (count%(91))*270, 0, 270,270, 32*(count%91), 64+(96*(Math.floor(count/91))), 32, 32) 
        // canvas_context.putImageData(statpixboxes2[count], 32, 32)

        // statpixboxes[count] = canvas_context.getImageData(0, 0, 32, 32)
        // canvas_context.putImageData(statpixboxes[count], 32*(count%13), 96+(96*(Math.floor(count/13))))


        // canvas_context.drawImage(frui, (count%(91))*270, 0, 270,270, 128+(32*(count%91)), 0+(96*(Math.floor(count/91))), 32, 32) 
        
        // canvas_context.putImageData(statpixboxes[count], 128+(32*(count%91)), 32+(96*(Math.floor(count/91))))
// 
        // canvas_context.putImageData(pix3, 32*(count%13), 128+(96*(Math.floor(count/13))))


        
        // canvas_context.putImageData(statpixboxes[count], 532+((count%13)*32), 0+(96*(Math.floor(count/13))))

        // canvas_context.putImageData(statpixboxes2[count], 532+((count%13)*32), 32+(96*(Math.floor(count/13))))

        // canvas_context.putImageData(pixboxes[count], 532+((count%13)*32), 64+(96*(Math.floor(count/13))))
...
function main() {
    ...
    if (start >= 1) {
        ...
        for (let t = 0; t < statpixboxes[8].data.length; t += 1) {
            if (t % 4 == 3) {
                ...
                if(!keysPressed['r']){
                    ...
                    if((t/statpixboxes[8].data.length)> ((count/9100)*1)){

                        // inputs.push(new Data(Math.random()))
                     inputs.push(new Data(statpixboxes[(filein-39)%(9100)].data[t] / 255))

                    }else{

                        inputs.push(new Data(statpixboxes[(filein-38)%(9100)].data[t] / 255))
                    }
                }
            }
            // pixboxes[count].data[t] = pix3.data[t]
        ...
        canvas_context.putImageData(pix3, 128+(32*(count%91)), 64+(32*(Math.floor(count/91))))
